...
//
// Architecture v13.94 : les résultats s'affichent en ligne dans la carte
// (plus de modal). Le tri place les dossiers « En cours » en premier.
...
/* Sélecteur du corps inline du dossier ouvert */
const CORPS_OUVERT  = '.fiche.ouvert .fiche-corps';
/* Sélecteur du badge du dossier ouvert */
const BADGE_OUVERT  = '.fiche.ouvert .droite .badge';
...
(async () => {
  ...
  // Le tri place les dossiers « En cours » en premier : nth-child(1) = TRAORE,
  // nth-child(2) = KOUASSI.
  {
    ...
    // Après tri « en cours en premier » : 1 = TRAORE (en cours), 2 = KOUASSI (rendu)
    r.check('le premier est marqué « En cours »',
            /En cours/.test(await page.textContent('#liste .fiche:nth-child(1)')), true);
    r.check('le second est marqué « Rendu »',
            /Rendu/.test(await page.textContent('#liste .fiche:nth-child(2)')), true);
  ...
  // nth-child(1) = TRAORE MOUSSA (en cours) après le tri.
  {
    ...
    await page.click('#liste .fiche:nth-child(1) .fiche-en-tete');
    await page.waitForTimeout(400);
    const corps = await page.textContent(CORPS_OUVERT);
    ...
    r.check('l\'état est « En cours »',
            (await page.textContent(BADGE_OUVERT)).trim(), 'En cours');
    r.check('aucune erreur JS', errors.length, 0);
    await ctx.close();
  }

  // ── 3. Un résultat rendu n'affiche PAS l'avertissement ───────────
  // nth-child(2) = KOUASSI AYA (rendu) après le tri.
  {
    ...
    await page.click('#liste .fiche:nth-child(2) .fiche-en-tete');
    await page.waitForTimeout(400);
    const corps = await page.textContent(CORPS_OUVERT);
    ...
    r.check('l\'état est « Rendu »',
            (await page.textContent(BADGE_OUVERT)).trim(), 'Rendu');
    r.check('aucune erreur JS', errors.length, 0);
    await ctx.close();
  }
  ...
  // ── 5. Le refus du serveur est respecté à l'écran ────────────────
  {
  ...
  // ── 6. Cloisonnement des rôles côté page ─────────────────────────
  {
  ...
  // ── 7. La connexion mène au bon endroit ──────────────────────────
  {
